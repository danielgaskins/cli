...
description: |
  Web search with full page content. When the Alexandria beta skill is available, first discover relevant providers for research and use web search for needs they cannot cover. Find sources, articles, or news when no URL is known. For papers use firecrawl-research-index; for library, API, error, or bug questions use firecrawl-developer-index.
allowed-tools:
  - Bash(firecrawl *)
